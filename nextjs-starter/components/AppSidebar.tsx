...
import { BarChart, ChevronLeft, ChevronRight, ExternalLink, FileText, Home, MessageCircle, Utensils } from "lucide-react"
              ...
              <SidebarGroupContent>
                {(
                  <div className="px-4 py-2 text-sm text-muted-foreground">
                    <p className="mb-2">
                      <strong>BAML</strong> is a prompting language that prioritizes <strong>type-safety</strong> and an exceptional <strong>developer experience</strong>.
                    </p>
                    <p>
                      It simplifies making great UX (e.g. streaming) by reducing <strong>boilerplate</strong> code, while ensuring high <strong>accuracy</strong> and <strong>consistency</strong> with LLMs.
                    </p>
                  </div>
                )}
              </SidebarGroupContent>
                      ...
                      <TooltipTrigger asChild>
                        <SidebarMenuItem>
                          <SidebarMenuButton
                            ...
                            isActive={isActive}
                            className={cn(
                              "flex items-center space-x-4 rounded-lg px-4 py-2 hover:bg-muted group",
                              isActive && "bg-muted"
                            )}
                          >
                            <>
                            <Link href={item.url} className="flex flex-col w-full items-start">
                            <div className='flex flex-row items-center'>
                              <item.icon className={cn("h-5 w-5", isActive && "text-primary")} />
                                ...
                                <div className="flex flex-wrap gap-2 gap-y-1 pl-6">
                  {item.tags.map((tag) => (
                    <Badge
                      key={tag}
                      variant="secondary"
                      style={{ backgroundColor: getTagColor(tag), opacity: 0.5 }}
