...
import type { Citation } from '@/baml_client';
import type { HookResult, HookResultData, HookResultPartialData, StreamingHookResult } from '@/baml_client/react/types';
...
import { AlertCircle, CheckCircle, CheckCircle2, Cog, Loader2, Rocket, XCircle } from 'lucide-react';
...
export const Content: React.FC<{
  ...
  answerAction: StreamingHookResult<'AnswerQuestion'>;
      ...
      <CardContent className='p-6'>
        <div className='flex flex-col gap-4 mb-6 text-sm text-gray-600'>
          <span>
            ...
            <Link className='text-blue-500 hover:underline font-medium' href={'https://en.wikipedia.org/wiki/SpaceX'}>
              SpaceX
            </Link>{' '}
...
const LoadPresetExample: React.FC<{ setQuestion: (value: string) => void }> = ({ setQuestion }) => {
  return (
    ...
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
      <Card className='mt-6 bg-muted shadow-lg rounded-xl overflow-hidden border-0'>
        <CardContent className='p-6'>
          ...
          {(status === 'success' && data) || (status === 'pending' && partialData) ? (
...
const RenderAnswer: React.FC<{ data: HookResultData<'AnswerQuestion'> | HookResultPartialData<'AnswerQuestion'> }> = ({
  data: { answer, answersInText },
}) => {
  if (!answersInText)
    return (
      <>
        <div className='min-h-[50px] text-gray-600'>Looking for citations...</div>
    ...
    return (
      <>
        <div className='min-h-[50px] text-gray-600'>
          Found {answersInText.length > 0 ? answersInText.length.toString() : ''} citations...
        </div>
        <div className='h-4 w-1/4 bg-gray-200 rounded mb-2 animate-pulse' />
        <div className='h-4 w-3/4 bg-gray-200 rounded mb-2 animate-pulse' />
        <div className='h-4 w-1/2 bg-gray-200 rounded animate-pulse' />
    ...
    <div className='text-lg leading-relaxed text-gray-800'>
      {parts.map((part, index) => {
        if (part.match(/^\[\d+\]$/)) {
          ...
          const citation = answersInText.find((c) => c?.number === Number.parseInt(citationNumber));
          ...
          return (
            <HoverCard key={citationNumber} openDelay={100} closeDelay={100}>
              <HoverCardTrigger asChild>
                <sup
                  className={clsx(
                    ...
                    context ? 'text-blue-600 hover:underline' : 'text-orange-500',
                  )}
                >
                  [{citationNumber}]
                </sup>
              ...
              >
                {context ? (
                  <>
                    <h4 className='font-semibold mb-2 text-gray-800'>{context.title}</h4>
                  ...
                  <p className='text-sm text-gray-600'>No exact match found for this citation.</p>
                )}
              </HoverCardContent>
            </HoverCard>
          );
...
const DebugPanel: React.FC<{
  ...
  const data = answerAction.isSuccess ? answerAction.data : answerAction.isPending ? answerAction.partialData : null;
  ...
  return (
    <Card className='w-full max-w-3xl mx-auto bg-white shadow-lg rounded-xl overflow-hidden border-0'>
      ...
      <CardContent className='p-0'>
        {answerAction.status === 'error' && answerAction.error && <ErrorPreview error={answerAction.error} />}
        <Tabs defaultValue='citations' className='w-full'>
          ...
          <TabsContent value='citations' className='p-4'>
            ...
            {answerAction.data?.answersInText?.map(
              (citation, index) => citation && <ShowCitation key={citation.number} citation={citation} />,
            )}
          </TabsContent>
  ...
  return (
    <Card className='mb-4 last:mb-0 overflow-hidden bg-white shadow-sm hover:shadow-md transition-shadow duration-200 rounded-lg border border-gray-200'>
      <CardHeader className='p-4 bg-gradient-to-r from-gray-50 to-gray-100 flex flex-row items-center justify-between'>
        <CardTitle className='text-lg font-semibold text-gray-800'>Citation [{citation.number}]</CardTitle>
        <TooltipProvider>
          <Tooltip>
            <TooltipTrigger asChild>
              ...
              >
                {isCitationFound ? <CheckCircle2 className='w-3 h-3 mr-1' /> : <XCircle className='w-3 h-3 mr-1' />}
