import type { Citation, Document } from '@/baml_client/types';

export const createWikipediaLink = (link: string, citedText: string) => {
  const encodedText = encodeURIComponent(citedText.replace(/\s+/g, ' ').trim());
  return `${link}#:~:text=${encodedText}`;
};

export const getCitationContext = (
  citation: Pick<Citation, 'documentTitle' | 'relevantTextFromDocument'>,
  documents: Document[],
) => {
  ...
  const citationIndex = lowerFullText.indexOf(lowerCitedText);
