...
export const Content: React.FC<{
  ...
  answer: HookResult<'AnalyzeBooks'>;
}> = ({ query, setQuery, answer }) => {
  const [bookColors, setBookColors] = useState<Record<string, string>>({});
  const [activeColorPicker, setActiveColorPicker] = useState<string | null>(null);
  const data = useMemo(
    () => (answer.isSuccess ? answer.data : answer.isPending ? answer.partialData : undefined),
    [answer],
  );
  ...
  return (
    <div className='min-h-screen bg-gradient-to-b from-blue-50 to-white py-12 px-4 sm:px-6 lg:px-8'>
      <div className='max-w-7xl mx-auto'>
        <h1 className='text-4xl font-bold text-center text-blue-900 mb-8'>Book Popularity Analyzer</h1>
          ...
          <div className='lg:col-span-2'>
            <Card className='mb-8 shadow-lg'>
              ...
              <CardContent>
                <div className='flex flex-col gap-4'>
                  <div>
                    <h3 className='text-sm font-semibold mb-2'>Try one of these examples:</h3>
                  ...
                  >
                    {answer.isPending ? <Loader2 className='mr-2 h-4 w-4 animate-spin' /> : 'Analyze Books'}
                  </Button>
                </div>
              </CardContent>
            </Card>
            ...
            <AnimatePresence>
              {answer.error && (
                ...
                >
                  <p className='text-red-500 text-sm'>Sorry! Something went wrong. Please try again.</p>
                </motion.div>
              )}
              {data && (
                <motion.div
                    ...
                    <CardContent>
                      <div className='flex flex-col gap-6'>
                        ...
                        <div className='space-y-8'>
                          <PopularityLineChart popularityData={data?.popularityOverTime} bookColors={bookColors} />
                          <RankingChart rankingData={data.popularityRankings} bookColors={bookColors} />
                          <WordCountChart wordCountData={data.wordCounts} bookColors={bookColors} />
                        </div>
                      </div>
                    </CardContent>
                  </Card>
