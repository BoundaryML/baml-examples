...
import { extractResume } from '../../actions/streamable-value';
    ...
    <div className='flex flex-col h-full w-full justify-center items-center mt-14'>
      ...
      <div className="w-full max-w-2xl p-6 md:p-8 lg:p-10">
        <Card className="bg-white rounded-lg shadow-md dark:bg-gray-800 dark:text-gray-200 w-[500px] h-[600px]">
          <CardHeader>
            <CardTitle className='flex flex-row items-center gap-x-4 relative'>{resume?.name || "John Doe"}{isLoading && <div className='absolute top-0 bottom-0 right-0'><ClipLoader color="gray" /></div>}
          </CardTitle>
          </CardHeader>
            ...
            <div>
              ...
              <ul className="grid grid-cols-2 gap-2 mt-2">
                {(resume?.skills ?? []).map((skill, num) => (
                  <li key={num} className="px-2 py-1 bg-gray-200 rounded-md dark:bg-gray-700">
                    {skill}
                  </li>
                ))}
              </ul>
              ...
              <ul className="space-y-2 mt-2">
                {(resume?.education ?? []).map((item, index) => (
                  <li key={index}>
                    ...
                    <p className="text-gray-500 dark:text-gray-400">{item.school}</p>
